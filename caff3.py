...
def checkout():
    ...
    breakdown_window.config(bg='lightgoldenrod')
    
    tk.Label(breakdown_window, text="Item", font=heading_font, bg='lightgreen').pack(pady=5)
    tk.Label(breakdown_window, text="Quantity", font=heading_font, bg='lightgreen').pack(pady=5)
    tk.Label(breakdown_window, text="Total Price", font=heading_font, bg='lightgreen').pack(pady=5)
    ...
    tk.Label(breakdown_window, text=f"Total Amount: ${total_amount:.2f}", font=bold_font, bg='lightcoral').pack(pady=10)
...
def open_menu():
    ...
    menu_window.config(bg='orange')

    # Create frames for each category
    frames = {}
    for i, category in enumerate(items.keys()):
        frames[category] = tk.LabelFrame(menu_window, text=category.capitalize(), padx=10, pady=10, bg='lightgreen', font=heading_font)
    ...
    for category, item_dict in items.items():
        ...
        for j, (item_name, item_info) in enumerate(item_dict.items()):
            label = tk.Label(frame, text=f"{item_name}: $ {item_info['price']}", bg='lightpink')
            label.pack(pady=5)
            
            quantity_label = tk.Label(frame, text=str(item_info["quantity"]), bg='lightpink')
            quantity_label.pack(pady=5)
            quantity_labels.setdefault(category, {})[item_name] = quantity_label
            
            increment_button = tk.Button(frame, text="+", command=lambda ctg=category, iname=item_name: increment(ctg, iname), bg='springgreen', font=bold_font)
            increment_button.pack(side="left", padx=5)
            
            decrement_button = tk.Button(frame, text="-", command=lambda ctg=category, iname=item_name: decrement(ctg, iname), bg='tomato', font=bold_font)
            decrement_button.pack(side="left", padx=5)

    # Checkout button on the menu page
    checkout_button = tk.Button(menu_window, text="Checkout", command=checkout, bg='royalblue', fg='white', font=bold_font)
    checkout_button.pack(pady=10)

# Button on the root window to open the menu page
open_menu_button = tk.Button(root, text="Open Menu", command=open_menu, bg='green', fg='white', font=heading_font)
open_menu_button.pack(pady=50, padx=20)

# Center the button in the frame
open_menu_button.place(relx=0.5, rely=0.5, anchor='center')
