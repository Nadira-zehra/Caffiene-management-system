...
def open_menu():
    ...
    menu_window.config(bg='lavender')
    ...
    for category, item_dict in items.items():
        ...
        for j, (item_name, item_info) in enumerate(item_dict.items()):
            ...
            increment_button = tk.Button(frame, text="+", command=lambda ctg=category, iname=item_name: increment(ctg, iname), bg='lightgreen', font=bold_font)
            increment_button.pack(side="left", padx=5)
            
            decrement_button = tk.Button(frame, text="-", command=lambda ctg=category, iname=item_name: decrement(ctg, iname), bg='lightcoral', font=bold_font)
            decrement_button.pack(side="left", padx=5)

    # Checkout button on the menu page
    checkout_button = tk.Button(menu_window, text="Checkout", command=checkout, bg='steelblue', fg='white', font=bold_font)
